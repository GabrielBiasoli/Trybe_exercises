...
SELECT 
    f.actor_id, a.first_name, f.film_id
FROM
    actor a
        INNER JOIN
...
-- 3. Exiba o id do cliente , nome e email dos primeiros 100 clientes, ordenados pelo nome em ordem decrescente, juntamente 
-- com o id do endereço e o nome da rua onde o cliente mora. Essas informações podem ser encontradas nas tabelas customer e address .
SELECT 
    c.customer_id,
    c.first_name,
    c.email,
...
SELECT 
    c.first_name, COUNT(a.address) AS address_quantity
FROM
    customer c
        INNER JOIN
    address a ON c.address_id = a.address_id
...
FROM
    film_actor fil_act
        INNER JOIN
    actor a ON fil_act.actor_id = a.actor_id
        INNER JOIN
    film f ON fil_act.film_id = f.film_id;
